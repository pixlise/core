...
func makeRGBUFileName(meta converterModels.ImageMeta) string {
...
func saveSpectrumTypeCounts(exp *protos.Experiment, data converterModels.OutputData) {
	readTypeIdx := 0
...
func copyImagesToOutput(contextImgDir string, outPath string, data converterModels.OutputData, jobLog logger.ILogger) (string, error) {
	defaultContextImage := ""
...
func setMatchedImageInfo(fromData converterModels.OutputData, toExperiment *protos.Experiment, jobLog logger.ILogger) error {
...
func makeDiscoFileName(meta converterModels.ImageMeta) string {
	//return fmt.Sprintf("MCC_MultiSpectral_%v_%v.png", meta.PMC, meta.LEDs)
	return path.Base(meta.FileName)
}

func (s *PIXLISEDataSaver) saveSpectrumMeta(meta converterModels.MetaData, detector *protos.Experiment_Location_DetectorSpectrum) error {
...
func (s *PIXLISEDataSaver) convertToOutputMeta(meta converterModels.MetaValue, labelIdx int32, saveMeta *protos.Experiment_Location_MetaDataItem) {
...
func (s *PIXLISEDataSaver) saveExperimentLocationItem(saveToExperiment *protos.Experiment, pmc int32, data converterModels.PMCData, hkHeaders []string, beamIJPMCAscending []int, jobLog logger.ILogger) error {
	...
	detectorSpectraLookup := map[string]converterModels.DetectorSample{}
...
func savePseudoIntensityRanges(exp *protos.Experiment, items []converterModels.PseudoIntensityRange) {
