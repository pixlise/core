...
func parseBeamLocations(rows [][]string, expectMultipleIJ bool, mainImagePMC int32) (converterModels.BeamLocationByPMC, error) {
	...
	result := converterModels.BeamLocationByPMC{}
...
func parseBeamLocationRow(row []string, headerLookup []pmcColIdxs, geom_corrIdx int32) (int32, converterModels.BeamLocation, error) {
	locData := converterModels.BeamLocation{}
	locData.IJ = map[int32]converterModels.BeamLocationProj{}
	...
	for _, ijCol := range headerLookup {
		...
		locData.IJ[ijCol.pmc] = converterModels.BeamLocationProj{I: float32(iVal), J: float32(jVal)}
	}
