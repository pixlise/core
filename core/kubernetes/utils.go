...
func (k *KubeHelper) RunPod(
	...
	wg.Add(1)
	go k.launchPod(&wg, cmd, args, env, volumes, volumeMounts, dockerImage, namespace, podnameprefix, labels, requestorUserId, background)
...
func (k *KubeHelper) launchPod(wg *sync.WaitGroup, cmd []string, args []string, env map[string]string, volumes []apiv1.Volume, volumeMounts []apiv1.VolumeMount, dockerImage string, namespace string, podnameprefix string, labels map[string]string, requestorUserId string, background bool) {
	defer wg.Done()

	podobj := k.getPodObject(cmd, args, env, volumes, volumeMounts, dockerImage, namespace, podnameprefix, labels, requestorUserId)
...
func (k *KubeHelper) getPodObject(cmd []string, args []string, env map[string]string, volumes []apiv1.Volume, volumeMounts []apiv1.VolumeMount, dockerImage string, namespace string, podnameprefix string, labels map[string]string, requestorUserId string) *apiv1.Pod {
	sec := apiv1.LocalObjectReference{Name: "api-auth"}
	rand.Seed(time.Now().UnixNano())
	podname := podnameprefix + utils.RandStringBytesMaskImpr(16)
	labels["owner"] = requestorUserId
	optional := true
	envvar := []apiv1.EnvVar{
