...
func (m BrukerData) Import(importPath string, pseudoIntensityRangesPath string, datasetID string, jobLog logger.ILogger) (*dataConvertModels.OutputData, string, error) {
	...
	instr := protos.ScanInstrument_BRUKER

	creator := params.CreatorUserId
	if len(creator) <= 0 {
		creator = specialUserIds.BrukerImport
	}
