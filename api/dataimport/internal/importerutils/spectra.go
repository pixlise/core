...
func splitSpectraCSVTables(data [][]string) ([][]string, [][]string) {
	...
	for idx, row := range data[1:] {
		if utils.SlicesEqual(row, csvMetaTableHeaderCols) || utils.SlicesEqual(row, csvMetaTableHeaderColsNoYellow) {
			dwellIdx = idx
			break
		}
	}
...
func parseSpectraCSVData(data [][]string, readType string, jobLog logger.ILogger) (dataConvertModels.DetectorSampleByPMC, error) {
	...
	if !utils.SlicesEqual(data[0], expMetaCols) {
		// Not new, maybe old?
		if utils.SlicesEqual(data[0], csvMetaTableHeaderColsNoYellow) {
			// Yes, old format
	...
	for idx, row := range data[1:] {
		if len(row) > len(tableAHeaderSample) && utils.SlicesEqual(row[0:len(tableAHeaderSample)], tableAHeaderSample) {
			// Found the A table!
			foundATable = true
			readingXYZTable = false
			channelTable = "A"
			...
			if utils.SlicesEqual(row, xyzTableHeader) {
