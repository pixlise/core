...
func createBeamLocation(isCalTarget bool, rsiPath string, rtt int64, outputBeamLocationPath string, logger logger.ILogger) (string, string, string, error) {
	...
	args := []string{bgtPath + "Geometry_PIXL_EM_Landing_25Jan2021.csv", fmt.Sprintf("\"%v\"", rsiPath), fmt.Sprintf("\"%v\"", outSurfaceTop), fmt.Sprintf("\"%v\"", outRXL)}
	if isCalTarget {
		args = append(args, "-t")
	}
	args = append(args, fmt.Sprintf("\"%v\"", outLog))
