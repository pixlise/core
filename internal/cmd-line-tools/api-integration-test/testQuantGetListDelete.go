...
func testQuantGetListDelete(apiHost string) {
	...
	u1.AddSendReqAction("Get quant log list (should fail, permissions dont allow)",
		...
		fmt.Sprintf(`{
			...
			"errorText": "View access denied for: QUANTIFICATION named \"Trial quant with Rh\" (id: %v)", "quantLogListResp":{}}`, quantId),
	)
	...
	u1.AddSendReqAction("Get quant from db (should fail, permissions dont allow)",
		...
		fmt.Sprintf(`{
			...
			"errorText": "View access denied for: QUANTIFICATION named \"Trial quant with Rh\" (id: %v)", "quantGetResp":{}}`, quantId),
	)
	...
	u1.AddSendReqAction("Get quant CSV from db (should fail, permissions dont allow)",
		...
		fmt.Sprintf(`{
			...
			"errorText": "View access denied for: QUANTIFICATION named \"Trial quant with Rh\" (id: %v)", "quantRawDataGetResp":{}}`, quantId),
	)
	...
	u1.AddSendReqAction("Get quant log (should fail, permissions dont allow)",
		...
		fmt.Sprintf(`{
			...
			"errorText": "View access denied for: QUANTIFICATION named \"Trial quant with Rh\" (id: %v)", "quantLogGetResp":{}}`, quantId),
	)
	...
	u1.AddSendReqAction("Delete quant (should fail, we're viewers!)",
		...
		fmt.Sprintf(`{"msgId":25,"status":"WS_NO_PERMISSION", "errorText": "Edit access denied for: QUANTIFICATION named \"Trial quant with Rh\" (id: %v)", "quantDeleteResp":{}}`, quantId),
	)
