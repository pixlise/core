...
func importImagesForDataset(datasetID string, instrument protos.ScanInstrument, srcBucket string, destDataBucket string, fs fileaccess.FileAccess, dest *mongo.Database) error {
	...
	for alignedIdx, img := range exprPB.AlignedContextImages {
		...
		go func(alignedIdx int, img *protos.Experiment_ContextImageCoordinateInfo) {
			...
			if err == nil {
				beamVersion := uint32(2) // We're assuming all PIXL datasets had their beams generated by v2
				if img.BeamVersion > 1 {
					beamVersion = img.BeamVersion // At time of conversion, the dataset files won't have this field so it should be 0... in case they do, use the one from the file
				}
