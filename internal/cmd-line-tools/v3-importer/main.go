...
func main() {
	...
	// Connect to mongo
	sourceMongoClient, _, err := mongoDBConnection.ConnectToMongo(sess, sourceMongoSecret, iLog, false)
	if err != nil {
		fatalError(err)
	}

	destMongoClient, _, err := mongoDBConnection.ConnectToMongo(sess, destMongoSecret, iLog, false)
	if err != nil {
		fatalError(err)
	}
