...
if [ "$db_reset" = "true" ]; then
    ...
    if [ $(ls -A $local_path) ]; then
        ...
        docker run --rm -d -v /$PWD/$local_path:/db-restore -v "$mongo_db_files:/data/db" -p 27017:27017 -h $(hostname) --name mongo-test "$mongoContainer" --replSet=test && sleep 4 && docker exec mongo-test mongo --eval "rs.initiate();" && sleep 2 && docker exec mongo-test mongorestore --gzip --db $db_name db-restore/
        exit 0
    fi
fi

echo "Starting docker MongoDB using previous volume..."
docker run --rm -d -v /$PWD/$local_path:/db-restore -v "$mongo_db_files:/data/db" -p 27017:27017 -h $(hostname) --name mongo-test "$mongoContainer" --replSet=test && sleep 4 && docker exec mongo-test mongo --eval "rs.initiate();"
