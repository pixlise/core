...
build-linux:
	...
	echo "sha: ${GITHUB_SHA}"
	GOOS=linux GOARCH=amd64 go build -ldflags "-X 'github.com/pixlise/core/api/services.ApiVersion=${BUILD_VERSION}' -X 'github.com/pixlise/core/api/services.GitHash=${GITHUB_SHA}'" -v -o ./_out/pixlise-api-linux ./internal/pixlise-api
